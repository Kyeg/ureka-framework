...
class VerificationFlow:
    ...
    def verify_ticket_protocol_verision(self, ticket_in: Ticket) -> None:
        ...
        if ticket_in.ticket_protocol_verision == ticket.TICKET_PROTOCOL_VERSION:
            logging.info("(Z-1) PASS: TICKET_PROTOCOL_VERSION")
        else:
            logging.error("(Z-1) ERROR: TICKET_PROTOCOL_VERSION")
            # return
    ...
    def verify_ticket_type(self, ticket_in: Ticket) -> None:
        ...
        if ticket_in.ticket_type == ticket.TYPE_INITIALIZATION_TICKET:
            logging.info("(Z-2) PASS: TYPE_INITIALIZATION_TICKET")
        elif ticket_in.ticket_type == ticket.TYPE_MANAGEMENT_TICKET:
            logging.info("(Z-2) PASS: TYPE_MANAGEMENT_TICKET")
        elif ticket_in.ticket_type == ticket.TYPE_ACCESS_PERMISSION_TICKET:
            logging.info("(Z-2) PASS: TYPE_ACCESS_PERMISSION_TICKET")
        elif ticket_in.ticket_type == ticket.TYPE_CHALLENGE_TICKET:
            logging.info("(Z-2) PASS: TYPE_CHALLENGE_TICKET")
        elif ticket_in.ticket_type == ticket.TYPE_RESPONSE_TICKET:
            logging.info("(Z-2) PASS: TYPE_RESONSE_TICKET")
        elif ticket_in.ticket_type == ticket.TYPE_KEY_EXCHANGE_TICKET:
            logging.info("(Z-2) PASS: TYPE_KEY_EXCHANGE_TICKET")
        else:
            logging.error("(Z-2) ERROR: WRONG_TICKET_TYPE")
            # return
    ...
    def verify_issuer_signature(
        ...
        if ticket_in.ticket_type == ticket.TYPE_MANAGEMENT_TICKET:
            if self._verify_issuer_signature_on_ticket(ticket_in, owner_pub_key):
                logging.info("(Z-4) PASS: ISSUER_SIGNATURE on MANAGEMENT_TICKET")
            else:
                logging.error("(Z-4) ERROR: ISSUER_SIGNATURE on MANAGEMENT_TICKET")
                # return
        elif ticket_in.ticket_type == ticket.TYPE_ACCESS_PERMISSION_TICKET:
            if self._verify_issuer_signature_on_ticket(ticket_in, owner_pub_key):
                # (Side Effect)
                ...
                logging.info("(Z-4) PASS: ISSUER_SIGNATURE on ACCESS_PERMISSION_TICKET")
            else:
                logging.error(
        ...
        if ticket_in.ticket_type == ticket.TYPE_CHALLENGE_TICKET:
            # To-Do: Return Ticket - to get DEVICE_ID after initialization
            logging.info("(N-1) PASS: ISSUER_SIGNATURE on CHALLENGE_TICKET")
        elif ticket_in.ticket_type == ticket.TYPE_RESPONSE_TICKET:
            ...
            if self._verify_issuer_signature_on_ticket(
                ...
                logging.info("(N-2) PASS: ISSUER_SIGNATURE on RESPONSE_TICKET")
            else:
                logging.error("(N-2) ERROR: ISSUER_SIGNATURE on RESPONSE_TICKET")
                # return
        elif ticket_in.ticket_type == ticket.TYPE_KEY_EXCHANGE_TICKET:
            # To-Do: Return Ticket - to get DEVICE_ID after initialization
            logging.info("(N-3) PASS: ISSUER_SIGNATURE on KEY_EXCHANGE_TICKET")
    ...
    def _verify_issuer_signature_on_ticket(
        ...
        try:
            ...
            logging.error("ERROR: NO SIGNATURE")
            return False
