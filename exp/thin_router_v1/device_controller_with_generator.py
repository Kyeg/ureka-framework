...
class DeviceController:
    ...
    ######################################################
    # Generate Different Ticket Types (not pure functions...)
    ######################################################
    def generate_initialization_ticket(self, holder_id: str):
        return TicketGenerator.generate_ticket("initialization")(holder_id=holder_id)

    def generate_query_ticket(self):
        return TicketGenerator.generate_ticket("query")()

    def generate_management_ticket(
        self, device_id: str, holder_id: str, task_scope: str
    ):
        return TicketGenerator.generate_ticket("management")(
            device_id=device_id,
            holder_id=holder_id,
            task_scope=task_scope,
            device_priv_key=self.device_priv_key,
        )

    def generate_access_permission_ticket(
        self, device_id: str, holder_id: str, task_scope: str
    ):
        return TicketGenerator.generate_ticket("access_permission")(
            device_id=device_id,
            holder_id=holder_id,
            task_scope=task_scope,
            device_priv_key=self.device_priv_key,
    ...
    def generate_challenge_ticket(self, device_id: str, holder_id: str):
        return TicketGenerator.generate_ticket("challenge")(
            device_id=device_id,
            holder_id=holder_id,
            device_priv_key=self.device_priv_key,
        )

    def generate_response_ticket(self, device_id: str, holder_id: str):
        return TicketGenerator.generate_ticket("response")(
            device_id=device_id,
            holder_id=holder_id,
            device_priv_key=self.device_priv_key,
        )

    def generate_key_exchange_ticket(self, device_id: str, holder_id: str):
        (new_ticket, self.current_session_key_byte) = TicketGenerator.generate_ticket(
            "key_exchange"
        )(
            device_id=device_id,
            holder_id=holder_id,
    ...
    def generate_command_ticket(self, device_id, holder_id, task_scope):
        return TicketGenerator.generate_ticket("command")(
            device_id=device_id, holder_id=holder_id, task_scope=task_scope
        )

    def generate_return_ticket(self):
        return TicketGenerator.generate_ticket("return")()
